...
<body class="bg-secondary text-white">
	...
	<%-- 	<c:set var="id" scope="session" value="${student.courses.id }" /> --%>
		...
		<table class="table table-hover table-dark">
			...
			<c:forEach var="tempCourse" items="${course}">
				...
				<!-- adding link to course with student id -->
				<c:url var="addCourse" value="/student/addCourse">
					<c:param name="courseId" value="${tempCourse.id }" />
					<c:param name="studentId" value="${student.id }" />
				</c:url>
				
				<!-- Set true if course id already exsists -->
				<c:set var="contains" value="false" />
				
				<c:forEach var="studentCourse" items="${student.courses }">
					<c:if test="${studentCourse.id == tempCourse.id }">
